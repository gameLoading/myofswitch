...
handle_desc_stats_request(struct ofconn *ofconn,
                          const struct ofp_header *request)
{
    static const char *default_mfr_desc = "DS Networks, Co.";
    static const char *default_hw_desc = "Open vSwitch AP";
...
static enum ofperr
handle_wparams_stats_request(struct ofconn* ofconn, const struct ofp_header* oh)
{
    VLOG_WARN("handle_wparams_request() called--2\n");
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "handle_wparams_request() called\n");
    closelog();

    ofp_wparams_stats_t wparamType;
    char deviceName[MAX_DEVICE_NAME_LEN];
    int res = ofputil_decode_wparams_stats_request(oh, &wparamType, deviceName, MAX_DEVICE_NAME_LEN);
    if (res == -1){
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "ofputil_decode_wparams_stats_request() fail\n");
        closelog();
    }

    openlog("ofproto.c", LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "wparamType : %d, deviceName : %s\n", wparamType, deviceName);
    closelog();
    
    
    char payload[MAX_JSON_SIZE];

    if (wparamType == DEVICES){
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "getAllWirelessDeviceWithJSON() call\n");
        closelog();
	    getAllWirelessDeviceWithJSON(payload, MAX_JSON_SIZE);    
        struct ofpbuf* buf;
        buf = ofputil_alloc_wparams_stats_reply(oh, DEVICES, 0, payload);
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "buf = ofputil_alloc_wparams_stats_reply(oh, DEVICES, 0, devices); called\n");
        closelog();
        ofconn_send_reply(ofconn, buf);
    }else{
        switch (wparamType)
        {
        case INFO:
            // handle_wparams_info_request(ofconn, oh, deviceName);
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getWirelessDeviceInfo(devices[0], info) call\n");
            closelog();
            getWirelessDeviceInfo(deviceName, payload, MAX_JSON_SIZE);
            break;
        case ASSOICATED:
            // handle_wparams_associcated_request(ofconn, oh, deviceName);
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getAssocicatedHosts(devices[0], info) call\n");
            closelog();
            getAssocicatedHosts(deviceName, payload, MAX_JSON_SIZE);
            break;
        case UCI:
            // handle_wparams_uci_request(ofconn, oh, deviceName);
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getWirelessUcis(deviceName, info, MAX_JSON_SIZE); call\n");
            closelog();
            getWirelessUcis(deviceName, payload, MAX_JSON_SIZE);
            break;
        case FREQLIST:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getFreqList(deviceName, info, MAX_JSON_SIZE); call\n");
            closelog();
            getFreqList(deviceName, payload, MAX_JSON_SIZE);
            break;
        case TXPOWERLIST:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getTxpowerList(deviceName, info, MAX_JSON_SIZE); call\n");
            closelog();
            getTxpowerList(deviceName, payload, MAX_JSON_SIZE);
            break;
        case COUNTRYLIST:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getCountryList(deviceName, payload, MAX_JSON_SIZE); call\n");
            closelog();
            getCountryList(deviceName, payload, MAX_JSON_SIZE);
            break;
        case SCAN:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getScannedSSIDList(deviceName, payload, MAX_JSON_SIZE); call\n");
            closelog();
            getScannedSSIDList(deviceName, payload, MAX_JSON_SIZE);
            break;
        case SURVEY:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "getFreqSurveyInfo(deviceName, payload, MAX_JSON_SIZE); call\n");
            closelog();
        ...
        default:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "wparams_stat_request type error : %d \n", wparamType);
            closelog();
            return 0;
        }

        openlog("ofproto.c", LOG_CONS, LOG_USER);    
        syslog(LOG_INFO, "call result : %s call\n", payload);
        closelog();

        struct ofpbuf* buf;
        buf = ofputil_alloc_wparams_stats_reply(oh, wparamType, deviceName, payload);
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "buf = ofputil_alloc_wparams_stats_reply(oh, DEVICES, 0, devices); called\n");
        closelog();
...
static enum ofperr
handle_wparams_set_config(struct ofconn* ofconn, const struct ofp_header* oh)
{
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "ofconn:%p oh:%p\n", ofconn, oh);
    closelog();

    VLOG_WARN("handle_wparams_set_config() called--3\n");
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    ...
    while(tlvs != NULL){
        ...
        case RESET:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "wifi reset of \n");
            closelog();
            InitialWirelessDevice();
            break;
        case SSID:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "set SSID of %s \n", tlvs->value);
            closelog();
            setSSID(deviceName, tlvs->value, MAX_DEVICE_NAME_LEN);
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "set SSID of %s over\n", tlvs->value);
            closelog();
            break;
        case TXPOWER:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "set Txpower of %d \n", atoi(tlvs->value));
            closelog();
            setTxPower(deviceName, atoi(tlvs->value));
            break;
        case UCIs:
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "set Ucis of %s \n", tlvs->value);
            closelog();
            setWirelessUcis(deviceName, tlvs->value);
            break;
        default:
            break;
        }
        if (tlvs->next == NULL){
            openlog("ofproto.c", LOG_CONS, LOG_USER);
            syslog(LOG_INFO, "this is last tlv  \n");
            closelog();
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    case OFPTYPE_WPARAMS_STATS_REQUEST:
        return handle_wparams_stats_request(ofconn, oh);
    case OFPTYPE_WPARAMS_SET_CONFIG:
        return handle_wparams_set_config(ofconn, oh);
