...
enum ofperr
handle_wparams_devices_request(struct ofconn* ofconn, const struct ofp_header *oh)
{
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    char devices[MAX_JSON_SIZE];
    syslog(LOG_INFO, "getAllWirelessDeviceWithJSON() call\n");
    closelog();
	getAllWirelessDeviceWithJSON(devices);

    struct ofpbuf* buf;
    
    buf = ofputil_alloc_wparams_stats_reply(oh, DEVICES, 0, devices);
    openlog("ofproto.c", LOG_CONS, LOG_USER);
...
enum ofperr handle_wparams_info_request(struct ofconn* ofconn, const struct ofp_header *oh, char *deviceName)
{
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    char info[MAX_JSON_SIZE];
    syslog(LOG_INFO, "getWirelessDeviceInfo(devices[0], info) call\n");
	getWirelessDeviceInfo(deviceName, info);
    syslog(LOG_INFO, "call result : %s call\n", info);

    struct ofpbuf* buf;
    
    buf = ofputil_alloc_wparams_stats_reply(oh, INFO, deviceName, info);
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "buf = ofputil_alloc_wparams_stats_reply(oh, DEVICES, 0, devices); called\n");
    closelog();
    ofconn_send_reply(ofconn, buf);
...
enum ofperr
handle_wparams_associcated_request(struct ofconn* ofconn, const struct ofp_header *oh, char *deviceName)
{
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    char info[MAX_JSON_SIZE];
    syslog(LOG_INFO, "getWirelessDeviceInfo(devices[0], info) call\n");
	getAssocicatedHosts(deviceName, info);
    syslog(LOG_INFO, "call result : %s call\n", info);
...
handle_wparams_request(struct ofconn* ofconn, const struct ofp_header* oh)
{
    ...
    ofp_wparams_t wparamType;
    char deviceName[MAX_DEVICE_NAME_LEN];
    ofputil_decode_wparams_stats_request(oh, &wparamType, deviceName);
    ...
    {
    case DEVICES:
        handle_wparams_devices_request(ofconn, oh);
        break;
    case INFO:
