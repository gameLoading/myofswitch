...
int setTxPower(char *ifname, int txpower) {
	...
	nla_put_u32(msg, NL80211_ATTR_WIPHY_TX_POWER_LEVEL, 0);
    ...
    err = 1;
    ret = nl_send_auto_complete(socket, msg);  // Send the message.
    printf("NL80211_CMD_SET_WIPHY sent %d bytes to the kernel.\n", ret);
    printf("Waiting for setting txpower to complete...\n");
    if (err < 0) {
        printf("WARNING: err has a value of %d.\n", err);
    }
    if (ret < 0) {
    ...
    printf("Setting txpower is done.\n");    
    ret = nl_recvmsgs_default(socket);  // Retrieve the kernel's answer. callback_dump() prints SSIDs to stdout.
    nlmsg_free(msg);
    nl_cb_put(cb);
