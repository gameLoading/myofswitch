...
ofpraw_from_ofphdrs(enum ofpraw *raw, const struct ofphdrs *hdrs)
{
    ...
    HMAP_FOR_EACH_WITH_HASH (raw_hdrs, hmap_node, hash, &raw_instance_map) {
        // if (hdrs->stat==21){
            // FILE *fp = fopen("/openvswitch.log", "a");
            // fprintf(fp, "version:%d, type:%d, stat:%d subType:%d length:%d\n", raw_hdrs->hdrs.version ,raw_hdrs->hdrs.stat, raw_hdrs->hdrs.subtype, raw_hdrs->hdrs_len);
            // fclose(fp);
            // VLOG_INFO("version:%d, type:%d, stat:%d subType:%d length:%d\n", raw_hdrs->hdrs.version ,raw_hdrs->hdrs.stat, raw_hdrs->hdrs.subtype, raw_hdrs->hdrs_len);
        // }
